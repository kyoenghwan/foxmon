...
async function main() {
    const sql = `
        ...
        BEGIN
            ...
            SET paid_points = v_new_paid,
                ...
                activity_points = v_new_paid + v_new_bonus
            WHERE id = p_user_id;
    ...
    console.log('⚡ Updating process_activity_point RPC in Supabase (unifying activity and ads points)...');
    const { data, error } = await supabaseAdmin.rpc('execute_sql', { sql });

    if (error) {
        console.error('❌ Error executing SQL:', error);
    } else {
        console.log('✅ Success! Redefined process_activity_point to unify point balance system.', data);
    }
}
