...
async function testSafeLookup(userId) {
    ...
    if (isUUID) {
        ...
        const { data } = await supabase.from('users').select('id, login_id, nickname').or(`login_id.eq.${rawUserId},login_id.eq.${normalizedUserId}`).maybeSingle();
        userData = data;
    }
...
async function run() {
    console.log("Result for UUID '872fa168-e834-4f7b-8a7e-e8c1c73955b0':", await testSafeLookup('872fa168-e834-4f7b-8a7e-e8c1c73955b0'));
    console.log("Result for login_id 'test1':", await testSafeLookup('test1'));
}
