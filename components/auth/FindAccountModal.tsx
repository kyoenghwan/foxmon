...
import { KeyRound, User, Check, Eye, EyeOff, Loader2, Smartphone, ShieldCheck } from 'lucide-react';

interface FindAccountModalProps {
  isOpen: boolean;
  onClose: () => void;
  defaultTab?: 'find-id' | 'find-pw';
}

export function FindAccountModal({ isOpen, onClose, defaultTab = 'find-id' }: FindAccountModalProps) {
  const [activeTab, setActiveTab] = useState<'find-id' | 'find-pw'>(defaultTab);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) { resetAllState(); onClose(); } }}>
      <DialogContent className="sm:max-w-[480px] p-6 bg-white rounded-3xl border border-gray-100 shadow-2xl">
        <DialogHeader className="flex flex-col items-center pb-3 border-b border-gray-100">
          <div className="w-12 h-12 bg-purple-50 rounded-2xl flex items-center justify-center mb-2">
            <ShieldCheck className="w-6 h-6 text-purple-600" />
          </div>
          <DialogTitle className="text-xl font-black text-gray-900 tracking-tight">계정 정보 찾기</DialogTitle>
          <p className="text-xs font-semibold text-gray-500 mt-1">휴대폰 본인인증으로 안전하게 계정을 찾으세요</p>
        </DialogHeader>

        {/* 탭 전환 */}
        <div className="grid grid-cols-2 gap-1.5 p-1.5 bg-gray-100/80 rounded-2xl my-2">
          <button
            type="button"
              ...
              <div className="space-y-4 animate-in fade-in">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {foundAccounts.map((acc, idx) => (
                    <div key={idx} className="p-3.5 bg-purple-50/60 border border-purple-100 rounded-2xl flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center">
                          <User className="w-4 h-4 text-purple-600" />
                        </div>
                        <span className="text-sm font-black text-purple-950">{acc.loginId}</span>
                      </div>
                ...
                <Button
                  ...
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white rounded-xl h-11 font-black text-xs mt-2"
                >
                  다시 찾기
                </Button>
              </div>
        ...
        {activeTab === 'find-pw' && (
          <div className="py-2">
              ...
              <form onSubmit={handlePwInputIdSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-11 rounded-xl text-sm font-medium border-gray-200 focus:ring-2 focus:ring-purple-500/20"
                    required
                  />
                </div>
                <Button
            ...
            {pwResetStep === 'verify' && (
              <div className="flex flex-col items-center space-y-4">
                <div className="p-2.5 bg-purple-50 border border-purple-100 rounded-xl text-xs font-bold text-purple-800 w-full text-center">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      className="h-11 rounded-xl text-sm font-medium border-gray-200 pr-10"
                      required
                    />
                    <button
                      type="button"
            ...
            {pwResetStep === 'done' && (
              <div className="text-center py-4 space-y-4 animate-in zoom-in-95">
                ...
                <p className="text-sm font-black text-gray-900">비밀번호가 정상 변경되었습니다.</p>
                <p className="text-xs text-gray-500 font-medium">새 비밀번호로 로그인해 주세요.</p>
                <Button
                  onClick={() => { resetAllState(); onClose(); }}
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white h-11 rounded-xl font-black text-xs"
                >
                  로그인하러 가기
                </Button>
