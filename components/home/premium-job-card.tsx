...
export function PremiumJobCard({ company, title, location, category, pay, image, tags, isBig, isSide, impactType = 'none', effectIntensity = 'medium', hideLogo = false, tier, id, customColor, bgOpacity, status }: PremiumJobCardProps) {
    ...
    return (
        <div className={`relative ${isBig ? 'h-full min-h-[292px]' : isSide ? 'aspect-[2/3]' : hideLogo ? 'aspect-[17/8]' : 'aspect-[3/2]'} w-full min-w-[150px] group p-[3px]`}>
            ...
            <Link 
                href={`/jobs/${id}`} 
                className={`relative h-full w-full rounded-[calc(0.75rem-3px)] overflow-hidden shadow-sm transition-all duration-300 ${isSide ? 'p-0.5' : 'p-0.5 sm:p-1.5 lg:p-1.5'} flex flex-col justify-between z-10 ${
                ...
                <div className={`flex flex-col h-full w-full relative z-10 ${isSide ? 'gap-1' : ''}`}>
                    ...
                    {isSide ? (
                        ...
                        <>
                            ...
                            {/* 상단: 지역 + 업체명 */}
                            <div className="flex items-center gap-1 sm:gap-1.5 pb-0.5 sm:pb-1 w-full overflow-hidden">
                                <div className={`flex items-center gap-0.5 sm:gap-1 shrink-0 border px-0.5 sm:px-1 py-[1px] leading-[1.1] font-bold rounded-[2px] text-[7px] sm:text-[10px] overflow-hidden ${
                                ...
                                <MarqueeText className={`font-black text-[10px] sm:text-[14px] lg:text-[15px] tracking-tight transition-colors line-clamp-1 leading-tight ${
                            ...
                            {/* 중간: 광고 제목 */}
                            <div className="flex-1 w-full flex flex-col justify-center relative overflow-hidden py-0.5">
                                <div className="w-full flex items-center gap-1 sm:gap-1.5">
                                    {category && (
                                        <span className={`shrink-0 border px-0.5 sm:px-1 py-[1px] rounded-[2px] text-[7px] sm:text-[11px] font-black ${
                                            isCyber ? 'bg-gray-800 text-gray-300 border-gray-700' : 'bg-gray-100 text-gray-600 border-gray-200'
                                        }`}>
                                            {category}
                                        </span>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <MarqueeText className={`text-[9px] sm:text-[13px] lg:text-[14px] leading-[1.3] font-bold tracking-tight px-0.5 sm:px-1 rounded-[2px] ${
                            ...
                            {/* 하단: 급여 및 뱃지 */}
                            <div className="flex items-end justify-between mt-auto w-full pt-0.5 sm:pt-1">
                                <div className="flex items-center text-[10px] sm:text-[14px] lg:text-[15px] font-bold text-gray-900 truncate tracking-tight gap-0.5 sm:gap-1.5">
                                    {payType && (
                                        <span className={`shrink-0 text-white text-[7px] sm:text-[10px] lg:text-[11px] px-1 sm:px-1.5 py-[1px] sm:py-0.5 rounded-sm shadow-sm ${
                                ...
                                <div className={`shrink-0 flex items-center px-1 sm:px-1.5 py-[1px] sm:py-0.5 rounded-sm text-[7px] sm:text-[10px] lg:text-[11px] font-black shadow-sm ${
                                    isCyber ? 'bg-cyan-900 text-cyan-200 border border-cyan-700' :
                                    isImpact ? 'bg-gradient-to-r from-yellow-100 to-amber-100 text-amber-700 border border-amber-200' : 
                                    'bg-gray-100 text-gray-700 border border-gray-300'
                                }`}>
                        ...
                        <div className="flex flex-col w-full h-full justify-between">
                            {/* 상단: 로고 50%, 상호명 50% */}
                            <div className="flex w-full h-[45%] gap-1 sm:gap-2 shrink-0">
                                {/* 로고 영역 (1.5:1 비율) */}
                                <div className="flex-1 min-w-0 bg-gray-50 flex items-center justify-center rounded-sm border border-gray-100 overflow-hidden shrink-0">
                                    {image ? (
                                        ...
                                        <div className="text-gray-300 font-black text-[7px] sm:text-[11px] bg-gray-100 w-full h-full flex items-center justify-center tracking-widest text-center leading-[1.1]">NO<br/>LOGO</div>
                                    )}
                                </div>
                                ...
                                <div className={`flex-1 min-w-0 flex flex-col justify-center space-y-0.5 sm:space-y-1`}>
                                    <div className="flex items-center w-full min-w-0">
                                        <div className={`flex items-center gap-0.5 sm:gap-1 shrink-0 border px-1 sm:px-1.5 py-[1px] sm:py-[1.5px] leading-[1.1] font-black rounded-[4px] text-[7px] sm:text-[10px] w-full overflow-hidden ${
                                            isCyber ? 'bg-gray-800 text-gray-300 border-gray-700' : 'bg-gray-100 text-gray-600 border-gray-200'
                                        }`}>
                                            <span className="shrink-0">{sido}</span>
                                            {sigungus && (
                                    ...
                                    <MarqueeText className={`font-black text-[10px] sm:text-[14px] lg:text-[15px] tracking-tight transition-colors line-clamp-2 leading-tight ${
                                        isCyber ? 'text-green-400 font-mono' : config.color
                                    }`}>
                                        {displayName}
                                    </MarqueeText>
                            ...
                            {/* 중간: 광고글 (멘트) */}
                            <div className="w-full relative overflow-hidden flex items-center gap-1 sm:gap-1.5 shrink-0 py-0.5">
                                {category && (
                                    <span className={`shrink-0 border px-0.5 sm:px-1 py-[1px] rounded-[2px] text-[7px] sm:text-[11px] font-black ${
                                        isCyber ? 'bg-gray-800 text-gray-300 border-gray-700' : 'bg-gray-100 text-gray-600 border-gray-200'
                                    }`}>
