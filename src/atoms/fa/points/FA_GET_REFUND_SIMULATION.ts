...
export const FA_GET_REFUND_SIMULATION = async (userId: string): Promise<RefundSimulationOutput> => {
  try {
    ...
    // 💡 1단계: 환불 대상 데이터 조회 (QA)
    const context = await QA_GET_REFUND_CONTEXT(userId);
    if (!context || context.activeHistory.length === 0) {
      return {
        success: false,
        totalRefundAmount: 0,
        refundFee: 0,
        ...
        error: 'NO_CHARGE_HISTORY'
      };
    }

    // 💡 2단계: 정밀 가치 정산 (RA)
    const calculation = RA_CALC_REFUND_PRO_RATA({
      activeHistory: context.activeHistory,
      refundFeeRatio: context.policy.refundFeeRatio
    });

    if (!calculation.isValid) {
      throw new Error(calculation.error || '환불 계산 중 오류가 발생했습니다.');
    }
    ...
    const reason = `총 ${activeCount}건의 충전 원금(${totalOriginalCash}원)을 기준으로, 사용하신 포인트만큼의 가비를 제외한 잔여 원금 합계에서 운영 수수료 ${calculation.refundFee.toLocaleString()}원(10%)을 차감한 금액입니다.`;
