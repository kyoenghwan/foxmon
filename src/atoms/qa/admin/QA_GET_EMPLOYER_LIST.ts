...
export async function QA_GET_EMPLOYER_LIST() {
    ...
    try {
        ...
        const { data, error } = await supabaseAdmin
            .from('users')
            .select('id, login_id, nickname, email, role, business_registration_number, is_business_verified, is_cert_verified, verified_ceo_name, verified_business_name, business_cert_image_url, business_type, verification_doc_url, created_at, paid_points, bonus_points, admin_memo')
            .eq('role', 'EMPLOYER')
